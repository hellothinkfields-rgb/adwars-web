...
export async function POST(req: NextRequest) {
  ...
  const { name, color, domain, charity_name, charity_url, user_id } = body;
  ...
  const cleanDomain = domain ? domain.trim().replace(/^https?:\/\//, '').replace(/\/.*$/, '') || null : null;
